...
public class AlertState : AIEnemyState
{
    enemyAI myEnemy;
    ...
    {
        // Rotamos al enemigo una vuelta completa en el timepo indicado por rotationTime
        ...
        {
            // Si aun estamos dando vueltas lanzamos un rayo desde una altura de 0.5m desde
            // la posición del enemigo hacia donde mira
            RaycastHit hit;

            if (Physics.Raycast(new Ray(new Vector3(myEnemy.transform.position.x, 0.5f, myEnemy.transform.position.z),
    ...
    // Como ya estamos en el estado Alert, no llamaremos nunca a esta función desde este estado
